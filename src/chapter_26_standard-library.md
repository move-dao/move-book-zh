...
[`vector`](./chapter_6_vector.md) type. The module is published under the
...
`向量`模块在原生类型[`向量`](./chapter_6_vector.md)上定义了许多操作。该模块以命名地址`Std`发布，并由许多原生函数以及在Move中定义的函数组成。此模块的API如下所示:

### 函数（Functions）

---------------------------------------------------------------------------

Create an empty [`vector`](./chapter_6_vector.md).
The `Element` type can be both a `resource` or `copyable` type.

创建一个空的[`向量`](./chapter_6_vector.md)。
...
Acquire an [immutable reference](./chapter_8_references.md) to the `i`th element of the vector `v`.  Will abort if
the index `i` is out of bounds for the vector `v`.

获取向量`v`的第`i`个元素的[不可变引用](./chapter_8_references.md)。如果索引`i`超出了向量`v`的范围，操作将会终止。
...
Acquire a [mutable reference](./chapter_8_references.md)
to the `i`th element of the vector `v`.  Will abort if
the index `i` is out of bounds for the vector `v`.

获取向量`v`的第`i`个元素的[可变引用](./chapter_8_references.md)。如果索引`i`超出了向量`v`的范围，操作将会终止。
