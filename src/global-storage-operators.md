...
Move 必须强制这种限制来保证全局存储引用不会出现空引用。[这节](#全局资源引用安全)包含更多读者感兴趣的细节。
...
- `m::f` 函数包含 `move_from<T>`、`borrow_global_mut<T>` 或 `borrow_global<T>` 指令调用
- `m::f` 函数调用了同模块内被 `acquires` 注解的 `m::g` 的函数
...
高级类型系统允许返回全局引用而尽可能不牺牲引用安全，我们将会在 Move 未来的迭代过程中考虑此事。选择目前的设计方式是因为它很好的平衡了语言表现力、复杂的注解和复杂的类型系统三者的关系。
